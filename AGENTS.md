...
Benchmarks use **JMH**. The harness is generated by an annotation processor
declared on the compiler plugin's `default-testCompile` execution — recent javac
does not discover processors from the classpath, so that declaration is load
bearing. Removing it makes the benchmarks compile but silently produce no
harness.
...
`src/test/java/mikera/*/performance/` holds JMH benchmarks, not correctness
tests. Run one with its `main`, or all of them via:

```bash
mvn -B test-compile dependency:build-classpath -Dmdep.outputFile=target/cp.txt
java -cp "target/classes;target/test-classes;$(cat target/cp.txt)" org.openjdk.jmh.Main <regex>
```

Two rules when writing one: **return the result** from a `@Benchmark` method so
JMH consumes it — a discarded result can be optimised away entirely, which is
what the old Caliper benchmarks did — and put fixture construction in `@Setup`,
choosing `Level.Iteration` where the original measured repeated mutation of one
object rather than a fresh one per call. `src/test/java/example/` holds runnable usage examples — keep them
