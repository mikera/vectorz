...
public class BlockTriangularSolver {
                               ...
                               final double temp[] )
    {
        ...
        final int offsetT = T.row0*T.original.columnCount()+M*T.col0;

        for( int i = 0; i < M; i += blockLength ) {
            int heightT = Math.min(T.row1-(i+T.row0),blockLength);

            int indexII = offsetT + T.original.columnCount()*(i+T.row0) + heightT*(i+T.col0);
            ...
            for( int j = 0; j < i; j += blockLength ) {
                ...
                for( int k = j; k < i; k += blockLength ) {
                    ...
                    int indexL = offsetT + T.original.columnCount()*(i+T.row0) + heightT*(k+T.col0);
                    int indexX = offsetT + T.original.columnCount()*(k+T.row0) + widthT*(j+T.col0);

                    blockMultMinus(dataT,dataX,temp,indexL,indexX,0,heightT,widthT,widthX);
                }

                int indexX = offsetT + T.original.columnCount()*(i+T.row0) + heightT*(j+T.col0);
                                   ...
                                   final boolean transT ,final boolean transB )
    {
        ...
        final int blockT_rows = Math.min(blockLength,T.original.rowCount()-T.row0);
        final int blockT_cols = Math.min(blockLength,T.original.columnCount()-T.col0);

        int offsetT = T.row0*T.original.columnCount()+blockT_rows*T.col0;
            ...
            if( upper ) {
                ...
                if ( transT ) {
                    ...
                    for( int i = B.row0; i < B.row1; i += blockLength ) {
                        ...
                        int offsetB = i*B.original.columnCount() + N*B.col0;
            ...
            if( upper ) {
                if ( transT ) {
                    for( int i = B.col0; i < B.col1; i += blockLength ) {
                        int offsetB = B.row0*B.original.columnCount() + Trows*i;
