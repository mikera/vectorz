...
public class TridiagonalDecompositionBlockHouseholder
    ...
    public BlockMatrix64F getT(BlockMatrix64F T) {
        if( T == null ) {
            T = new BlockMatrix64F(A.rowCount(),A.columnCount(),A.blockLength);
        } else {
            if( T.rowCount() != A.rowCount() || T.columnCount() != A.columnCount() )
    ...
    public BlockMatrix64F getQ(BlockMatrix64F Q, boolean transposed) {
        Q = BlockMatrix64HouseholderQR.initializeQ(Q, A.rowCount() , A.columnCount()  , A.blockLength , false);

        int height = Math.min(A.blockLength,A.rowCount());
        V.reshape(height,A.columnCount(),false);
        tmp.reshape(height,A.columnCount(),false);
    ...
    public boolean decompose(BlockMatrix64F orig) {
        ...
        int N = orig.columnCount();

        for( int i = 0; i < N; i += A.blockLength ) {
//            System.out.println("-------- triag i "+i);
            int height = Math.min(A.blockLength,A.rowCount()-i);
                                       ...
                                       D1Submatrix64F C )
    {
        ...
        for( int i = C.row0+blockLength; i < C.row1; i += blockLength ) {
            ...
            int indexA = A.row0*A.original.columnCount() + (i-C.row0+A.col0)*heightA;

            for( int j = i; j < C.col1; j += blockLength ) {
                int widthC = Math.min( blockLength , C.col1 - j );

                int indexC = i*C.original.columnCount() + j*heightC;
                int indexB = B.row0*B.original.columnCount() + (j-C.col0+B.col0)*heightA;
    ...
    private void init( BlockMatrix64F orig ) {
        ...
        int height = Math.min(A.blockLength,A.rowCount());
        V.reshape(height,A.columnCount(),A.blockLength,false);
        tmp.reshape(height,A.columnCount(),A.blockLength,false);

        if( gammas.length < A.columnCount() )
            gammas = new double[ A.columnCount() ];
