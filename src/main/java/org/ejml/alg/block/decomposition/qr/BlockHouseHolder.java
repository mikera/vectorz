                                            ...
                                            final D1Submatrix64F A , final int col , final double gamma )
    {
        ...
        for( int j = col+1; j < width; j++ ) {
            ...
            for( int i = A.row0; i < A.row1; i += blockLength ) {
                ...
                int indexU = i*A.original.columnCount() + height*A.col0 + col;
                int indexA = i*A.original.columnCount() + height*A.col0 + j;
                                                ...
                                                final D1Submatrix64F A , final int col , final double gamma )
    {
        ...
        for( int colStartJ = A.col0 + blockLength; colStartJ < A.col1; colStartJ += blockLength ) {
            ...
            for( int j = 0; j < widthJ; j++ ) {
                ...
                int indexU = i*A.original.columnCount() + height*A.col0 + col;
                int indexA = i*A.original.columnCount() + height*colStartJ + j;
                                             ...
                                             final int row , final int colStart , final double gamma )
    {
        ...
        for( int i = row+1; i < height; i++ ) {
            ...
            for( int j = A.col0; j < A.col1; j += blockLength ) {
                ...
                int indexU = A.row0*A.original.columnCount() + height*j + row*width;
                int indexA = A.row0*A.original.columnCount() + height*j + i*width;
...
//                plusScale_row(blockLength,);
                ...
                int indexU = A.row0*A.original.columnCount() + heightU*A.col0 + row*width;
                int indexA = blockStart*A.original.columnCount() + heightA*A.col0 + i*width;
    ...
    public static double innerProdCol( int blockLength, D1Submatrix64F A,
        ...
        for( int i = A.row0; i < A.row1; i += blockLength ) {
            ...
            int indexA = i*A.original.columnCount() + height*colBlockA + colA;
            int indexB = i*A.original.columnCount() + height*colBlockB + colB;
...
//                    for( int k = 0; k < height; k++ ) {
    ...
    public static void divideElementsCol( final int blockLength ,
        ...
        for( int i = Y.row0; i < Y.row1; i += blockLength ) {
            ...
            int index = i*Y.original.columnCount() + height*Y.col0 + col;
    ...
    public static double computeTauAndDivideRow( final int blockLength ,
        ...
        for( int j = startJ; j < Y.col1; j += blockLength ) {
            ...
            int index = Y.row0*Y.original.columnCount() + height*j + row*width;
    ...
    public static void initializeW( final int blockLength,
        ...
        for( int i = W.row0; i < W.row1; i += blockLength ) {
            ...
            int indexW = i*W.original.columnCount() + heightW*W.col0;
            int indexY = i*Y.original.columnCount() + heightW*Y.col0;
                                 ...
                                 final int col , final double []temp , final double beta )
    {
        ...
        final int colsW = W.original.columnCount();
        ...
        for( int i = Y.row0; i < Y.row1; i += blockLength ) {
            ...
            int indexV = i*Y.original.columnCount() + heightW*Y.col0 + col;
...
//                for( int k = 0; k < heightW; k++ ,
                                      ...
                                      final D1Submatrix64F C )
    {
        ...
        for( int i = Y.row0; i < Y.row1; i += blockLength ) {
            ...
            for( int j = B.col0; j < B.col1; j += blockLength ) {
                ...
                int indexC = (i-Y.row0+C.row0)*C.original.columnCount() + (j-B.col0+C.col0)*heightY;

                for( int k = Y.col0; k < Y.col1; k += blockLength ) {
                    int indexY = i*Y.original.columnCount() + k*heightY;
                    int indexB = (k-Y.col0+B.row0)*B.original.columnCount() + j*widthY;
                                          ...
                                          D1Submatrix64F C )
    {
        ...
        for( int j = B.col0; j < B.col1; j += blockLength ) {
            ...
            int indexC = C.row0*C.original.columnCount() + (j-B.col0+C.col0)*widthA;

            for( int k = A.row0; k < A.row1; k += blockLength ) {
                int heightA = Math.min( blockLength , A.row1 - k );

                int indexA = k*A.original.columnCount() + A.col0*heightA;
