...
public class NaiveDeterminant {
    ...
    {
        PermuteArray perm = new PermuteArray( mat.columnCount() );
    ...
    public static double recursive( DenseMatrix64F mat )
    {
        ...
        for(int i = 0; i < mat.rowCount(); i++) {
            DenseMatrix64F minorMat = new DenseMatrix64F(mat.rowCount()-1,mat.rowCount()-1);

            for(int j = 1; j < mat.rowCount(); j++) {
                for(int k = 0; k < mat.rowCount(); k++) {
