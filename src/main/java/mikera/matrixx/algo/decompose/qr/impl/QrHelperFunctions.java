...
/**
 * <p>
 * Contains different functions that are useful for computing the QR
 * decomposition of a matrix.
 * </p>
 * 
 * <p>
 * Two different families of functions are provided for help in computing
 * reflectors. Internally both of these functions switch between normalization
 * by division or multiplication. Multiplication is most often significantly
 * faster than division (2 or 3 times) but produces less accurate results on
 * very small numbers. It checks to see if round off error is significant and
 * decides which one it should do.
 * </p>
 * 
 * <p>
 * Tests were done using the stability benchmark in jmatbench and there doesn't
 * seem to be any advantage to always dividing by the max instead of checking
 * and deciding. The most noticeable difference between the two methods is with
 * very small numbers.
 * </p>
 * 
 * @author Peter Abeles
...
public class QrHelperFunctions {
  ...
  public static void divideElements(int j, int numRows, double[] u, int startU,
      double u_0) {
    // double div_u = 1.0/u_0;
    //
    // if( Double.isInfinite(div_u)) {
  ...
  public static void divideElements_Brow(int j, int numRows, double[] u,
      double b[], int startB, double u_0) {
    // double div_u = 1.0/u_0;
    //
    // if( Double.isInfinite(div_u)) {
    for (int i = j; i < numRows; i++) {
  ...
  public static void divideElements_Bcol(int j, int numRows, int numCols,
      double[] u, double b[], int startB, double u_0) {
    // double div_u = 1.0/u_0;
    //
    // if( Double.isInfinite(div_u)) {
  ...
  public static double computeTauAndDivide(int j, int numRows, double[] u,
      int startU, double max) {
  ...
  /**
   * Normalizes elements in 'u' by dividing by max and computes the norm2 of the
   * normalized array u. Adjust the sign of the returned value depending on the
   * size of the first element in 'u'. Normalization is done to avoid overflow.
   * 
   * <pre>
   * for i=j:numRows
   *   u[i] = u[i] / max
   ...
   * </p>
   */
  public static void rank1UpdateMultR(Matrix A, double u[], double gamma,
  ...
  public static void rank1UpdateMultR(Matrix A, double u[], int offsetU,
      double gamma, int colA0, int w0, int w1, double _temp[]) {
    // for( int i = colA0; i < A.numCols; i++ ) {
    // double val = 0;
    //
    // for( int k = w0; k < w1; k++ ) {
   ...
   * </p>
   */
  public static void rank1UpdateMultL(Matrix A, double u[], double gamma,
      int colA0, int w0, int w1) {
