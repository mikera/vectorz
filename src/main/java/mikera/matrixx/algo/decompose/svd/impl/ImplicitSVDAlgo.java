...
/**
 * <p>
 * Computes the QR decomposition of a bidiagonal matrix. Internally this matrix
 * is stored as two arrays. Shifts can either be provided to it or it can
 * generate the shifts on its own. It optionally computes the U and V matrices.
 * This comparability allows it to be used to compute singular values and
 * associated matrices efficiently.<br>
 * <br>
 ...
 * 
 * <p>
 * Note: To watch it process the matrix step by step uncomment commented out
 * code.
 * </p>
 * 
 * @author Peter Abeles
 */
public class ImplicitSVDAlgo {
  ...
  protected Matrix Ut;
  protected Matrix Vt;
  // number of times it has performed an implicit step, the most costly part of
  // the
  // algorithm
  ...
  public ImplicitSVDAlgo(boolean fastValues) {
    this.fastValues = fastValues;
  }

  public ImplicitSVDAlgo() {
  ...
  public boolean _process() {
    ...
    while (x2 >= 0) {
      ...
      if (x1 == x2) {
        ...
        // There are analytical solutions to this case. Just compute them
        // directly.
        resetSteps();
        eigenBB_2x2(x1);
        setSubmatrix(x2, x2);
  ...
  /**
   * Here the lambda in the implicit step is determined dynamically. At first it
   * selects zeros to quickly reveal singular values that are zero or close to
   * zero. Then it computes it using a Wilkinson shift.
   */
  private void performDynamicStep() {
    ...
    if (findingZeros) {
      ...
      // overflow/underflow
      // is to have a common scale between the wilkinson shift and the implicit
      // single step
      // What happens if you don't is that when the wilkinson shift returns the
      // value it
      // computed it multiplies it by the scale twice, which will cause an
      // overflow
      double scale = computeBulgeScale();
  ...
  /**
   * Shifts are performed based upon singular values computed previously. If it
   * does not converge using one of those singular values it uses a Wilkinson
   * shift instead.
   */
  ...
  /**
   * Tells it to process the submatrix at the next split. Should be called after
   * the current submatrix has been processed.
   */
   ...
   */
  public void performImplicitSingleStep(double scale, double lambda,
      boolean byAngle) {
  ...
  /**
   * Multiplied a transpose orthogonal matrix Q by the specified rotator. This
   * is used to update the U and V matrices. Updating the transpose of the
   * matrix is faster since it only modifies the rows.
   * 
   * 
  ...
  protected void updateRotator(Matrix Q, int m, int n, double c, double s) {
    int rowA = m * Q.columnCount();
    int rowB = n * Q.columnCount();
  ...
  /**
   * Selects the Wilkinson's shift for B<sup>T</sup>B. See page 410. It is
   * guaranteed to converge and converges fast in practice.
   * 
  ...
  /**
   * Checks to see if either the diagonal element or off diagonal element is
   * zero. If one is then it performs a split or pushes it off the matrix.
   * 
  ...
  /**
   * If there is a zero on the diagonal element, the off diagonal element needs
   * pushed off so that all the algorithms assumptions are two and so that it
  ...
  /**
   * It is possible for the QR algorithm to get stuck in a loop because of
   * symmetries. This happens more often with larger matrices. By taking a
   * random step it can break the symmetry and finish.
   */
