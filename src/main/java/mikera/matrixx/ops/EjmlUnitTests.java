...
public class EjmlUnitTests {
  ...
  /**
   * Checks to see if every element in A is countable. A doesn't have any
   * element with a value of NaN or infinite.
   * 
   * @param A Matrix
   */
  public static void assertCountable(Matrix A) {
    for (int i = 0; i < A.rowCount(); i++) {
      for (int j = 0; j < A.columnCount(); j++) {
        ...
        assertTrue(!Double.isInfinite(A.get(i, j)), "Infinite found at " + i
            + " " + j);
      }
    }
  }
  ...
  /**
   * <p>
   * Checks to see if each element in the matrix is within tolerance of each
   * other:
   * </p>
   * 
   * <p>
   ...
   * 
   * <p>
   * In addition if an element is NaN or infinite in one matrix it must be the
   * same in the other.
   * </p>
   * 
   * @param A Matrix A
    ...
    for (int i = 0; i < A.rowCount(); i++) {
      for (int j = 0; j < A.columnCount(); j++) {
        ...
        if (Double.isNaN(valA)) {
          assertTrue(Double.isNaN(valB), "At (" + i + "," + j + ") A = " + valA
              + " B = " + valB);
        } else if (Double.isInfinite(valA)) {
          assertTrue(Double.isInfinite(valB), "At (" + i + "," + j + ") A = "
              + valA + " B = " + valB);
        } else {
          double diff = Math.abs(valA - valB);
          assertTrue(diff <= tol, "At (" + i + "," + j + ") A = " + valA
              + " B = " + valB);
        }
      }
    }
  ...
  /**
   * <p>
   * Checks to see if each element in the matrices are within tolerance of each
   * other and countable:
   * </p>
   * 
   * <p>
  ...
  public static void assertEquals(Matrix A, Matrix B, double tol) {
    ...
    for (int i = 0; i < A.rowCount(); i++) {
      for (int j = 0; j < A.columnCount(); j++) {
        ...
        assertTrue(!Double.isNaN(valA) && !Double.isNaN(valB), "At (" + i + ","
            + j + ") A = " + valA + " B = " + valB);
        assertTrue(!Double.isInfinite(valA) && !Double.isInfinite(valB), "At ("
            + i + "," + j + ") A = " + valA + " B = " + valB);
        assertTrue(Math.abs(valA - valB) <= tol, "At (" + i + "," + j
            + ") A = " + valA + " B = " + valB);
      }
    }
  }
  ...
  public static void assertEqualsTrans(Matrix A, Matrix B, double tol) {
    ...
    for (int i = 0; i < A.rowCount(); i++) {
      for (int j = 0; j < A.columnCount(); j++) {
        ...
        assertTrue(!Double.isNaN(valA) && !Double.isNaN(valB), "A(" + i + ","
            + j + ") = " + valA + ") B(" + j + "," + i + ") = " + valB);
        assertTrue(!Double.isInfinite(valA) && !Double.isInfinite(valB), "A("
            + i + "," + j + ") = " + valA + ") B(" + j + "," + i + ") = "
            + valB);
        assertTrue(Math.abs(valA - valB) <= tol, "A(" + i + "," + j + ") = "
            + valA + ") B(" + j + "," + i + ") = " + valB);
      }
    }
  }
