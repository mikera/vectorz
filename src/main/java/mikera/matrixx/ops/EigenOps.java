...
public class EigenOps {
  /**
   * <p>
   * Given matrix A and an eigen vector of A, compute the corresponding eigen
   * value. This is the Rayleigh quotient.<br>
   * <br>
   ...
   * </p>
   * <p>
   * NOTE: If there is another eigenvalue that is very similar to the provided
   * one then there is a chance of it converging towards that one instead. The
   * larger a matrix is the more likely this is to happen.
   * </p>
   * 
  ...
  /**
   * <p>
   * Computes the dominant eigen vector for a matrix. The dominant eigen vector
   * is an eigen vector associated with the largest eigen value.
   * </p>
   * 
   * <p>
   * WARNING: This function uses the power method. There are known cases where
   * it will not converge. It also seems to converge to non-dominant eigen
   * vectors some times. Use at your own risk.
   * </p>
   * 
  ...
  /**
   * <p>
   * Generates a bound for the largest eigen value of the provided matrix using
   * Perron-Frobenius theorem. This function only applies to non-negative real
   * matrices.
   * </p>
   * 
   * <p>
   * For "stochastic" matrices (Markov process) this should return one for the
   * upper and lower bound.
   * </p>
   * 
   * @param A Square matrix with positive elements. Not modified.
   * @param bound Where the results are stored. If null then a matrix will be
   *          declared. Modified.
   * @return Lower and upper bound in the first and second elements
   *         respectively.
   */
  public static double[] boundLargestEigenValue(Matrix A, double[] bound) {
    if (A.rowCount() != A.columnCount())
   ...
   * If an eigenvalue is imaginary then zero is stored in its place.
   * </p>
   * 
   * @param eig An eigenvalue decomposition which has already decomposed a
   *          matrix.
  ...
  /**
   * <p>
   * Puts all the real eigenvectors into the columns of a matrix. If an
   * eigenvalue is imaginary then the corresponding eigenvector will have zeros
   * in its column.
   * </p>
   * 
   * @param eig An eigenvalue decomposition which has already decomposed a
